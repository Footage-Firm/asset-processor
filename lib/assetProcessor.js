...
function AssetProcessor(config) {
    ...
    if (this.config.targets.images && !this.config.targets.images.extensions) {
        this.config.targets.images.extensions = ['.png','.jpg','.jpeg','.gif','.bmp','.svg'];
    }
...
function _getFilesRelativeToRoot(mainRoot, activeRoot, targets, normalizedFullPath, callback) {
    var relative = path.relative(mainRoot, activeRoot);
    ...
    _getFiles(targets, activeRoot, normalizedFullPath, function(err, files) {
        if (normalizedFullPath) {
            ...
            var relativeFiles = _.map(files || [], function (file) {
...
AssetProcessor.prototype.uploadJavaScriptToCdn = function(excludeSourceMap, callback) {
    ...
    async.auto({
        ...
        minify: ['getPath', function(next, results) {
            ...
            var uglifyOptions = excludeSourceMap ? {} : {outSourceMap: self.s3.urlWithBucket(mapPath)};
        ...
        uploadMap: ['minify', function(next, results) {
            var mapPath = results.getPath.replace('.js','.map');
            if (!excludeSourceMap) {
    ...
    async.auto({
        ...
        process: ['files', function(next, results) {
            ...
            files = files.filter(function(file) {
                ...
                var included = (preference || []).some(function(preference) {
                    return file.toLowerCase() === preference;
                });

                // if extension matches and not excluded, keep the file!
                return extensionMatch && (!excluded || included); 
            });
...
function _firstDirNameFromRootPath(rootPath, defaultDirName, callback) {
    var folder = path.sep+(_firstDirName(rootPath) || defaultDirName);
    if (callback) {
...
function _rebaseUrls(self, file, css) {
    ...
    var imgRebaseRoot = _getImageFolder(self.config.imagesRootRelative); //relative url path to the root for rebased (remote) image assets from within S3 bucket
    var extrasRoot = self.config.extrasRoot; //full path to the local root for extra assets
    var extrasRebaseRoot = _getExtrasFolder(self.config.extrasRootRelative); //relative url path to the root for rebased (remote) extra assets from within S3 bucket
...
function _applyImportMappings(filePath, mappings, callback) {
    ...
    async.auto({
        ...
        replaceMappings: ['readFile', function(next, results) {
            var css = results.readFile;

            Object.keys(mappings || {}).forEach(function(importedRoot) {
                var search = _escapeRegExp(importedRoot);
                var replacement = mappings[importedRoot];
        ...
        writeFile: ['replaceMappings', function(next, results) {
            var remappedCss = results.replaceMappings;
...
function _toGitHubApiSource(sourceUrl) {
    ...
    var gitHubBlobUrlRegex = /^https?:\/\/github\.com\/([^\/]+)\/([^\/]+)\/blob\/[^\/]+\/(.*)$/;
    var match = gitHubBlobUrlRegex.exec(sourceUrl);
    var org;
    var repo;
    var path;
    if (match) {
