...
function _checkRelativeToRoot(self, type, callback) {
    ...
    async.auto({
        ...
        getFolder: [function(next) {
            if (type === 'image') {
                _getImageFolder(self.imagesRootRelative, next);
            } else {
                _getExtrasFolder(self.extrasRootRelative, next);
            }
        }],
...
function _importFromGit(sourceUrl, destPath, accessToken, callback) {
    ...
    req.on('end', function() {
        var err = !success ? new Error('Error requesting '+apiUrl+'; status code '+response.statusCode) : null;
        callback(err);
    });
