 ...
 * @param callback
 * @param [outputDir] {string} directory to write the file (defaults to javascriptsRoot)
 */
AssetProcessor.prototype.saveJavaScriptToFile = function(callback, outputDir) {
    ...
    async.auto({
        ...
        saveFile: ['getSource', function (next, results) {
            ...
            if (source) {
                const basePath = outputDir ? outputDir : self.config.javascriptsRoot;
 ...
 * @param callback
 * @param [outputDir] {string} directory to write the file (defaults to stylesheetsRoot)
 */
AssetProcessor.prototype.saveCssToFile = function(callback, outputDir) {
    ...
    async.auto({
        ...
        saveFile: ['getSource', function (next, results) {
            ...
            if (source) {
                const basePath = outputDir ? outputDir : self.config.stylesheetsRoot;
                const fullLocalPath = basePath + '/' + results.getSource.getPath;
                relativeUrl = '/' + results.getSource.getPath;
 ...
 * @param {object} [opts] Options passed from the asset processor command
 * @param {boolean} [opts.skipJs] If true processing JavaScript files will be skipped
 * @param {boolean} [opts.outputDir] Optional directory to write processed css and js files
 * @param {function} callback function(err, results)
 * @return {object} result
...
AssetProcessor.prototype.processAssets = function(opts, callback) {
    ...
    async.auto({
        processJs: [function(next) {
            if (!opts.skipJs) {
                self.saveJavaScriptToFile(next, opts.outputDir);
            } else {
                next();
            }
        }],
        processCss: [function(next) {
            self.saveCssToFile(next, opts.outputDir);
        }]
    }, function(err, results) {
...
function _rebaseUrls(self, file, css) {
    ...
    const extrasRoot = self.config.extrasRoot;
    const imagesRoot = self.config.imagesRoot;
        ...
        if (!externalHttpRegex.test(url) && !base64Regex.test(url)) {
            ...
            if (absoluteUrlRegex.test(normalizedUrl)) {
                ...
                rebasedUrl = path
                    ...
                    .replace(cssRoot, '')
                    .replace(extrasRoot, path.basename(extrasRoot))
                    .replace(imagesRoot, path.basename(imagesRoot));

                //Ensure rebase is not relative
                if (rebasedUrl[0] != '/') {
