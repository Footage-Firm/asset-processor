 ...
 * @param {bool|object|function} makePrivateOrHeaders whether or not to make the file private on S3; or the full set of heders to use.
...
S3.prototype.putFile = function(sourcePath, targetPath, makePrivateOrHeaders, callback) {
    ...
    const untilEnd = () => { return fileUpload || numTries > maxTries };
    const untilIterator = (untilNext) => {
    ...
    async.until(untilEnd, untilIterator, callback);
};
